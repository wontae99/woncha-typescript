...
function CommentListItem({ comment, isMine, onShowForm, onDelete }) {
  const time = new Date();
  ...
  writtenTime = new Date(comment.date);
  timeDiff = Math.floor((currentTime - writtenTime) / 1000); //second
  ...
  return (
    ...
    >
      <div className="flex justify-between w-full">
        <Link className="flex group rounded-lg p-1 hover:bg-slate-200" href={`/user/${comment.user._id}`}>
          <Image
